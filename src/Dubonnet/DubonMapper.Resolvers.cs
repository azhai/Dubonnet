...
namespace Dubonnet
{
    public static partial class DubonMapper
    {
        ...
        public static partial class Resolvers
        {
            ...
            private static readonly ConcurrentDictionary<RuntimeTypeHandle, DubonProperty[]> TypePropertiesCache = new ConcurrentDictionary<RuntimeTypeHandle, DubonProperty[]>();
            ...
            public static DubonProperty KeyProperty(Type type) => KeyProperty(type, out _);
            ...
            /// <returns>The key property for <paramref name="type"/>.</returns>
            public static DubonProperty KeyProperty(Type type, out bool isIdentity)
            {
                if (!TypeKeyPropertyCache.TryGetValue(type.TypeHandle, out var keyProperty))
                {
                    var DubonProperty = _keyPropertyResolver.Resolve(type, out isIdentity);
                    keyProperty = new KeyPropertyInfo(DubonProperty, isIdentity);
                    TypeKeyPropertyCache.TryAdd(type.TypeHandle, keyProperty);
                }

                isIdentity = keyProperty.IsIdentity;

                LogReceived?.Invoke($"Resolved property '{keyProperty.DubonProperty}' (Identity: {isIdentity}) as key property for '{type.Name}'");
                return keyProperty.DubonProperty;
            }
            ...
            public static DubonProperty ForeignKeyProperty(Type sourceType, Type includingType, out ForeignKeyRelation foreignKeyRelation)
            {
                ...
                LogReceived?.Invoke($"Resolved property '{foreignKeyInfo.DubonProperty.PropertyInfo}' ({foreignKeyInfo.Relation.ToString()}) as foreign key between '{sourceType.Name}' and '{includingType.Name}'");
                return foreignKeyInfo.DubonProperty;
            }
            ...
            public static IEnumerable<DubonProperty> Properties(Type type)
            {
            ...
            /// </summary>
            /// <param name="propertyInfo">The <see cref="DubonProperty"/> to get the column name for.</param>
            /// <returns>The column name in the database for <paramref name="propertyInfo"/>.</returns>
            public static string Column(DubonProperty propertyInfo)
            {
            ...
            /// </summary>
            /// <param name="propertyInfo">The <see cref="DubonProperty"/> to get the data column for.</param>
            /// <returns>The data column for <paramref name="propertyInfo"/>.</returns>
            public static Tuple<string, Type> DataColumn(DubonProperty propertyInfo)
            {
                var key = propertyInfo.PropertyKey;
