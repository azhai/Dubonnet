...
namespace Dubonnet.Internal
{
    ...
    internal class SqlExpression<T>
    {
        ...
        protected virtual object VisitUnary(UnaryExpression expression)
        {
            switch (expression.NodeType)
            {
                case ExpressionType.Not:
                    ...
                    if (memberExpression != null && DubonMapper.Resolvers.Properties(memberExpression.Expression.Type).Any(p => p.PropertyInfo.Name == (string)o))
                    {
                        o = $"{o} = '1'";
                    }
        ...
        {
            return DubonMapper.Resolvers.Column(new DubonProperty(expression.Expression.Type, expression.Member as PropertyInfo));
        }
